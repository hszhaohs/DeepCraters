...
def train_sup(label_loader, model, criterions, optimizer, epoch, args):
    ...
    for i in range(len(label_iter)):
        input, attr, target, _, _ = next(label_iter)
        ...
        attr_var = torch.autograd.Variable(attr)
        target_var = torch.autograd.Variable(target)
        
        # compute output
        output = model(input_var, attr_var)
        ...
        (prec1, prec5), pred = accuracy(output.data, target, topk=(1, 5))
...
def train_mt(label_loader, unlabel_loader, model, model_teacher, criterions, optimizer, epoch, args, ema_const=0.95, weight_mt=8.0):
    ...
    for i in range(len_iter):
        ...
        try:
            input, attr, target, input1, attr1 = next(label_iter)
        except StopIteration:
            label_iter = iter(label_loader)     
            input, attr, target, input1, attr1 = next(label_iter)
        input_ul, attr_ul, _, input1_ul, attr1_ul = next(unlabel_iter)
        sl = input.shape
        su = input_ul.shape
        ...
        attr1_var = torch.autograd.Variable(attr1)
        input_ul_var = torch.autograd.Variable(input_ul)
        attr_ul_var = torch.autograd.Variable(attr_ul)
        input1_ul_var = torch.autograd.Variable(input1_ul)
        attr1_ul_var = torch.autograd.Variable(attr1_ul)
        input_concat_var = torch.cat([input_var, input_ul_var])
        attr_concat_var = torch.cat([attr_var, attr_ul_var])
        input1_concat_var = torch.cat([input1_var, input1_ul_var])
        attr1_concat_var = torch.cat([attr1_var, attr1_ul_var])
       
        target_var = torch.autograd.Variable(target)

        # compute output
        if args.is_inception:
            output, aux_output = model(input_concat_var, attr_concat_var)
            with torch.no_grad():
                output1, aux_output1 = model_teacher(input1_concat_var, attr1_concat_var)
    
            output_label = output[:sl[0]]
            output_label_aux = aux_output[:sl[0]]    ##
            ...
            loss_ce_aux = criterion(output_label_aux, target_var) /float(sl[0])    ##
            loss_cl = criterion_mse(output, output1) /float(args.num_classes * batch_size)
            loss_cl_aux = criterion_mse(aux_output, aux_output1) /float(args.num_classes * batch_size)    ##
    
            reg_l1 = cal_reg_l1(model, criterion_l1)
    
            loss = loss_ce + args.weight_l1 * reg_l1 + weight_cl * weight_mt * loss_cl + 0.001*loss_ce_aux + 0.001*loss_cl_aux
        
        else:
            output = model(input_concat_var, attr_concat_var)
            with torch.no_grad():
                output1 = model_teacher(input1_concat_var, attr1_concat_var)
        ...
        # measure accuracy and record loss
        (prec1, prec5), pred = accuracy(output_label.data, target, topk=(1, 5))
        (prec1_t, prec5_t), pred_t = accuracy(output1_label.data, target, topk=(1, 5))
        losses.update(loss_ce.item(), input.size(0))
...
def validate(val_loader, model, criterions, args, mode = 'valid'):
    ...
    with torch.no_grad():
        for i, (input, attr, target, _, _) in enumerate(val_loader):
            sl = input.shape
            batch_size = sl[0]
            target = target.cuda(async=True)
            input_var = torch.autograd.Variable(input)
    ...
    return top1.avg, losses.avg, pred_all, prob_all
